...
const SHOPPING_GUIDE: Record<string, { buy: string, why: string }> = {
  'GENERAL_FEED': { buy: 'Balanced Fertilizer (10-10-10)', why: 'A safe, all-purpose feed for general maintenance.' },
  'VEG_GROW': { buy: 'High-Nitrogen Liquid Feed (e.g. Fish Emulsion)', why: 'Fuels rapid, lush leaf growth for greens and herbs.' },
  'BLOOM_BOOST': { buy: 'High-Phosphorus Granular Feed', why: 'Provides energy for heavy flower and fruit production.' },
  'ACID_LOVERS': { buy: 'Acidifying Fertilizer', why: 'Lowers soil pH so these specific plants can absorb nutrients without iron deficiency.' },
  'LOW_FEED': { buy: 'Nothing! (Or half-strength Liquid Kelp)', why: 'These herbs thrive in poor soil. Too much food dilutes their flavor and causes leggy growth, so feeding is rarely needed.' }
};
    ...
    <div id="shopping-print-portal" className="fixed inset-0 z-[9999] bg-white text-black overflow-y-auto">
      ...
      <div className="p-8 print:p-0 max-w-3xl mx-auto font-sans">
        <div className="border-b-4 border-emerald-600 pb-4 mb-8">
          <h1 className="text-4xl font-black text-slate-900 mb-2">{gardenName} Shopping List</h1>
          <p className="text-lg text-slate-600 font-medium">Generated by FloraSync</p>
        </div>
              ...
              return (
                <div key={feed} className="print-item">
                  ...
                  <div className={`border-2 rounded-2xl p-6 ${feed === 'GENERAL_FEED' ? 'bg-slate-50 border-slate-300 border-dashed' : 'bg-white border-slate-200'}`}>
                    ...
                    <span className="text-slate-500 font-semibold">{data.count === 0 ? 'General Fallback' : `${data.count} Plant${data.count !== 1 ? 's' : ''}`}</span>
                  </div>
                  
                  <div className="space-y-4">
                    <div className="bg-emerald-50 rounded-xl p-4 border border-emerald-100">
                      <h3 className="text-xs font-bold text-emerald-800 uppercase tracking-wider mb-1">What to Buy</h3>
                      <p className="text-xl font-bold text-emerald-900">{guide.buy}</p>
                    </div>
                    
                    <div className="grid grid-cols-2 gap-6">
                      <div>
                        <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Why do I need this?</h3>
                        <p className="text-sm text-slate-700 leading-relaxed">{guide.why}</p>
                      </div>
                      <div>
                        <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">For which plants?</h3>
                      ...
                      return (
                        <div key={feed}>
                          ...
                          <div className="bg-white dark:bg-slate-900/50 p-3 rounded-lg border border-slate-100 dark:border-slate-800 shadow-sm">
                            ...
                            <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed">{guide.why}</p>
                          </div>
                          </div>
                        </div>
                      );
