  ...
  return (
    <Card className="mb-4">
      <div className="flex items-center gap-4 mb-4">
        <div className="relative">
          {currentUser.imageUrl ? (
            <img src={currentUser.imageUrl} alt="Profile" onError={(e) => { e.currentTarget.onerror = null; e.currentTarget.src = FALLBACK_IMAGE; }} className="w-16 h-16 rounded-full object-cover border-2 border-primary-500" />
          ) : (
            <div className="w-16 h-16 rounded-full bg-primary-100 dark:bg-primary-900/30 flex items-center justify-center text-2xl font-bold text-primary-700 dark:text-primary-400 border-2 border-primary-500">
              {currentUser.name.charAt(0).toUpperCase()}
            </div>
          )}
          <label className="absolute bottom-0 right-0 bg-primary-500 text-white rounded-full p-1.5 cursor-pointer hover:bg-primary-600 transition-colors shadow-md text-xs leading-none flex items-center justify-center">
      ...
      {!showPasswordForm ? (
        ...
        <form onSubmit={handleChangePassword} className="bg-surface-50 dark:bg-surface-900/50 p-4 rounded-xl border border-surface-200 dark:border-surface-700 mb-4 flex flex-col gap-3">
          <div>
